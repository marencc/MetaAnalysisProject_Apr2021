...
raw <- getGEO(GEO = "GSE8479")
# [["GSE8479_series_matrix.txt.gz"]]
gse <- getGEO(GEO = "GSE8479")[["GSE8479_series_matrix.txt.gz"]]
...
head(refseq)
refseq[is.na(refseq)] <- as.character("NA")
anno <- lapply(c("ENTREZID","SYMBOL","GENENAME"), 
                function(x) mapIds(org.Hs.eg.db, refseq, x, "REFSEQ"))
columns(org.Hs.eg.db)

annodf <- data.frame(PROBEID = fData(raw[[1]])[,1], 
                      ACCNUM = refseq, ENTREZID = anno[[1]],
                      SYMBOL = anno[[2]], GENENAME = anno[[3]])
head(annodf)
fData(raw[[1]]) <- annodf
fData(raw[[1]])[,3] ## ENTREZID
fData(raw[[1]])[,4] ## SYMBOL
...
oligo::boxplot(log2(exprs(male)), target = "core",
...
individual <- factor(Biobase::pData(male)$id)
...
write.csv(table, file = "res_GSE8479M.csv")


# Exploring Results
head(table)
nrow(subset(table, P.Value < 0.001))  # 408
nrow(subset(table, adj.P.Val < 0.05))  # 161
