...
head(pData(gse)$data_processing, 3)
...
pData(dat)$individual <- c("A", "A", "B", "B", "C", "C", "D", "D", "E", "E", "F", "F")
pData(dat)$individual <- factor(pData(dat)$individual)  # male: A,F  female: B,C,D,E
...
pData(female)
individual <- factor(Biobase::pData(female)$individual)
timepoint <- factor(Biobase::pData(female)$timepoint)
id <- factor(Biobase::pData(female)$id)
...
write.csv(contr.fit, file = "res_139258_F.csv")
