...
#### Importing Data ####
counts <- read.table("counts.sickle.txt", sep = "\t", header = TRUE)
head(counts)
View(counts)
rownames(counts) <- counts$Geneid
colnames(counts) <- sub(".sickle.fastq.sort.bam", "", colnames(counts))
...
write.table(counts, "countmat.sickle.txt", sep = "\t", col.names = TRUE)

# checking the data
c <- read.table("countmat.sickle.txt", sep = "\t", header = TRUE)
head(c)
dim(c)  # 60664    56
