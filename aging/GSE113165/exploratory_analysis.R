...
# keep columns needed
sampleInfo <- sampleInfo[,c("Run", "age", "Sample.Name", "sex", "Subject", "susceptibility", "Time")]
head(sampleInfo, 3)
#          Run age Sample.Name  sex Subject          Time
# 1 SRR7007949 old  GSM3098323 male    3005  pre bed rest
# 2 SRR7007950 old  GSM3098324 male    3005 post bed rest
# 3 SRR7007951 old  GSM3098325 male    3008  pre bed rest

sampleInfo$Subject <- paste0("S", sampleInfo$Subject)
sampleInfo$sex <- str_sub(sampleInfo$sex, 1, 1)
sampleInfo$Time <- ifelse(str_detect(sampleInfo$Time, "pre"), "pre", "post")
sampleInfo$Time <- relevel(as.factor(sampleInfo$Time), ref = "pre")
sampleInfo$age <- relevel(as.factor(sampleInfo$age), ref = "young")
sampleInfo$sex <- relevel(as.factor(sampleInfo$sex), ref = "m")
summary(sampleInfo)
# Run                age        Sample.Name        sex    Subject            Time   
# Length:56          young:18   Length:56          m:26   Length:56          pre :28  
# Class :character   old  :38   Class :character   f:30   Class :character   post:28  
# Mode  :character              Mode  :character          Mode  :character

head(sampleInfo, 3)
#          Run age Sample.Name sex Subject Time
# 1 SRR7007949 old  GSM3098323   m   S3005  pre
# 2 SRR7007950 old  GSM3098324   m   S3005 post
# 3 SRR7007951 old  GSM3098325   m   S3008  pre

# matching the order with count matrix
...
# writing out a table
write.table(sampleInfo, "pdata.txt", sep = "\t", row.names = TRUE)
...
head(df)
sampleTime <- ifelse(df$Samples %in% c("SRR1554535", "SRR1554556", "SRR1554561"), "adult", "fetus")
df = data.frame(df, Condition = sampleTime)
...
ggplot(dataGG, aes(PC1, PC2)) +
    geom_point(aes(shape = Individual, colour = Time)) +
...
library(DESeq2)
deseq_data <- DESeqDataSetFromMatrix(countData = counts, colData = sampleInfo, design = ~ Time)  # ref = "adult"
colData(deseq_data)
colnames(deseq_data)
deseq_data
