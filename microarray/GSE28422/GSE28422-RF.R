...
#. Create subsets ----
# female
female <- raw_data[,raw_data$gender == "female"]
keep <- c("T1", "T3")
filter <- colnames(female)[female$timepoint %in% keep]
length(filter)
...
# Take a look
Biobase::exprs(female)[1:5, 1:5]

# log2
exp_raw <- log2(Biobase::exprs(female))
...
dataGG <- data.frame(PC1 = PCA_raw$x[,1], PC2 = PCA_raw$x[,2],
                     individual = pData(female)$id,
                     timepoint_id = pData(female)$individual,
                     timepoint = pData(female)$timepoint
)
...
arrayQualityMetrics(expressionset = female,
                    outdir = "QCreport",
...
eset <- oligo::rma(female, normalize = FALSE)
...
dataGG <- data.frame(PC1 = PCA$x[,1], PC2 = PCA$x[,2],
                     individual = pData(female)$id,
                     id.time = pData(female)$individual,
                     timepoint = pData(female)$timepoint
)
...
abline(v = man_threshold, col = "coral4", lwd = 3)
...
write.csv(table_T1T3, "Res_GSE28422_RF_T1T3.csv")
...
#. Volcano plot ----
volcano_names <- ifelse(abs(fit_T1T3$coefficients)>=1,
                        fit_T1T3$genes$SYMBOL, NA)

volcanoplot(fit_T1T3, coef = 1L, style = "p-value", highlight = 100,
            names = volcano_names,
