...
dim(exprs(manfiltered))  # 49032    94
...
nrow(probe_stats) # 2066: clusters that map to multiple gene symbols → remove
...
for (i in unique(group)) {
    ...
    write.csv(table, file = paste0("res_GSE48278gse", i, ".csv"))
    print(head(table))
}
...
result_files <- list.files(pattern = "gse...csv")
result_files
