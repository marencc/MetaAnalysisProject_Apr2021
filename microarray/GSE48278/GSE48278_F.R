...
keep <- c("A","C","D","E","F")
filter <- colnames(female)[female$grp.short %in% keep]
length(filter)
...
# Outlier
pData(female)[c(1,2,17,18,19,20,39,40,41,42),]

# exclude outliers
outlier_samples <- c(1,2,17,18,19,20,39,40,41,42)
...
dataGG <- data.frame(PC1 = PCA$x[,1], PC2 = PCA$x[,2],
                     individual = pData(female)$individual,
                     gender = pData(female)$gender,
                     timepoint = pData(female)$timepoint,
                     group = pData(female)$grp.short
)
...
write.csv(table_A, "Res_DEG_GSE48278_FA.csv")
...
write.csv(table_C, "Res_DEG_GSE48278_FC.csv")
...
write.csv(table_D, "Res_DEG_GSE48278_FD.csv")
...
write.csv(table_E, "Res_DEG_GSE48278_FE.csv")
...
write.csv(table_F, "Res_DEG_GSE48278_FF.csv")
